...
public class ProcessosView {
    ...
    // Exibe apenas o último concurso da loteria selecionada
    private void carregarConcursos() {
        ...
        try {
            ConcursoDAO concursoDAO = new ConcursoDAO();
            ConcursoNumeroSorteadoDAO numeroDAO = new ConcursoNumeroSorteadoDAO();
            Concurso ultimoConcurso = concursoDAO.buscarUltimoConcursoPorLoteria(loteria.getId());
            if (ultimoConcurso != null) {
                List<ConcursoNumeroSorteado> dezenas = numeroDAO.listarNumerosPorConcurso(ultimoConcurso.getId());
                String dezenasStr = dezenas.stream()
                        .map(n -> String.format("%02d", n.getNumero()))
                        .collect(Collectors.joining(" - "));
                dadosTabela.add(new ConcursoRow(ultimoConcurso, dezenasStr));
